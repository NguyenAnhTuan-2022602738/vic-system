...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Fetch stock data from vnstock')
    parser.add_argument('--symbol', default='VIC', help='Stock symbol (default: VIC)')
    ...
    os.makedirs(f"{args.output}/raw", exist_ok=True)
    os.makedirs(f"{args.output}/processed", exist_ok=True)
    ...
    try:
        df_features = process_features(df, output_dir=args.output)
        print(f"\nHoan thanh! Du liệu san sang de train model.")
